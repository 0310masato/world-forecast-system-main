...
function assertSafeOutput(output) {
  for (const { name, pattern } of RESTRICTED_OUTPUT_PATTERNS) {
    if (pattern.test(output)) {
      throw new Error(`Dry-run output contains restricted content: ${name}.`);
    }
  }
}
...
function loadWriteDryRunHelpers() {
...
try {
  const {
    makeTaskBoardHandoffWriteDryRunRequest,
    runTaskBoardHandoffWriteDryRun,
  } = loadWriteDryRunHelpers();
  const request = makeTaskBoardHandoffWriteDryRunRequest();
  const result = runTaskBoardHandoffWriteDryRun(request);
  const output = JSON.stringify(result, null, 2);

  assertSafeOutput(output);
  console.log(output);
} catch (error) {
  console.error(`[codex-app-server-runtime-write-dry-run] ${sanitize(error.stack || error.message || error)}`);
  process.exitCode = 1;
}
