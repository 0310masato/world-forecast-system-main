...
function assertSafeOutput(output) {
  for (const { name, pattern } of RESTRICTED_OUTPUT_PATTERNS) {
    if (pattern.test(output)) {
      throw new Error(`Write plan output contains restricted content: ${name}.`);
    }
  }
}
...
try {
  ...
  const output = JSON.stringify(writePlanDraft, null, 2);

  assertSafeOutput(output);
  console.log(output);
} catch (error) {
  console.error(`[codex-app-server-runtime-write-plan] ${sanitize(error.stack || error.message || error)}`);
  process.exitCode = 1;
}
