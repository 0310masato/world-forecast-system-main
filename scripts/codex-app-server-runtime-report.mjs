...
function assertSafeOutput(output) {
  for (const { name, pattern } of RESTRICTED_OUTPUT_PATTERNS) {
    if (pattern.test(output)) {
      throw new Error(`Report output contains restricted content: ${name}.`);
    }
  }
}
...
function loadRuntimeReportHelpers() {
  const scaffoldModule = loadTypeScriptModule(
    path.join(projectRoot, 'lib', 'codex-app-server-runtime', 'scaffold.ts'),
  );
  const reportModule = loadTypeScriptModule(
    path.join(projectRoot, 'lib', 'codex-app-server-runtime', 'report.ts'),
  );
...
try {
  ...
  console.error(`[codex-app-server-runtime-report] ${sanitize(error.stack || error.message || error)}`);
  process.exitCode = 1;
}
