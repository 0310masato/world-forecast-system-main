...
function assertSafeOutput(output) {
  for (const { name, pattern } of RESTRICTED_OUTPUT_PATTERNS) {
    if (pattern.test(output)) {
      throw new Error(`Write executor contract output contains restricted content: ${name}.`);
    }
  }
}
...
function loadWriteExecutorContractHelpers() {
  ...
  const writePlanModule = loadTypeScriptModule(
    path.join(projectRoot, 'lib', 'codex-app-server-runtime', 'write-plan.ts'),
  );
  const writeApplyPreflightModule = loadTypeScriptModule(
    path.join(projectRoot, 'lib', 'codex-app-server-runtime', 'write-apply-preflight.ts'),
  );
  const writeExecutorContractModule = loadTypeScriptModule(
    path.join(projectRoot, 'lib', 'codex-app-server-runtime', 'write-executor-contract.ts'),
  );
  ...
  return {
    ...
    makeTaskBoardHandoffWritePlanDraft:
      writePlanModule.makeTaskBoardHandoffWritePlanDraft,
    makeTaskBoardHandoffWriteApplyPreflight:
      writeApplyPreflightModule.makeTaskBoardHandoffWriteApplyPreflight,
    makeTaskBoardHandoffWriteExecutorContractDraft:
      writeExecutorContractModule.makeTaskBoardHandoffWriteExecutorContractDraft,
  };
}
...
try {
  const {
    ...
    makeTaskBoardHandoffWriteApplyPreflight,
    makeTaskBoardHandoffWriteExecutorContractDraft,
  } = loadWriteExecutorContractHelpers();
  ...
  const writePlanDraft = makeTaskBoardHandoffWritePlanDraft(
  ...
  const applyPreflight = makeTaskBoardHandoffWriteApplyPreflight(
    writePlanDraft,
  );
  const executorContractDraft =
    makeTaskBoardHandoffWriteExecutorContractDraft(applyPreflight);
  const output = JSON.stringify(executorContractDraft, null, 2);

  assertSafeOutput(output);
  console.log(output);
} catch (error) {
  console.error(`[codex-app-server-runtime-write-executor-contract] ${sanitize(error.stack || error.message || error)}`);
  process.exitCode = 1;
}
