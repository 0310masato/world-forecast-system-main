...
function assertSafeOutput(output) {
  for (const { name, pattern } of RESTRICTED_OUTPUT_PATTERNS) {
    if (pattern.test(output)) {
      throw new Error(`Approval request output contains restricted content: ${name}.`);
    }
  }
}
...
function loadWriteApprovalRequestHelpers() {
...
try {
  const {
    ...
    runTaskBoardHandoffWriteDryRun,
    makeTaskBoardHandoffWriteApprovalRequestDraft,
  } = loadWriteApprovalRequestHelpers();
  ...
  const output = JSON.stringify(approvalRequestDraft, null, 2);

  assertSafeOutput(output);
  console.log(output);
} catch (error) {
  console.error(`[codex-app-server-runtime-write-approval-request] ${sanitize(error.stack || error.message || error)}`);
  process.exitCode = 1;
}
