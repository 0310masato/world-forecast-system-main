...
function assertSafeOutput(output) {
  for (const { name, pattern } of RESTRICTED_OUTPUT_PATTERNS) {
    if (pattern.test(output)) {
      throw new Error(`Write apply preflight output contains restricted content: ${name}.`);
    }
  }
}
...
function loadWriteApplyPreflightHelpers() {
...
try {
  const {
    ...
    makeTaskBoardHandoffWritePlanDraft,
    makeTaskBoardHandoffWriteApplyPreflight,
  } = loadWriteApplyPreflightHelpers();
  ...
  const output = JSON.stringify(applyPreflight, null, 2);

  assertSafeOutput(output);
  console.log(output);
} catch (error) {
  console.error(`[codex-app-server-runtime-write-apply-preflight] ${sanitize(error.stack || error.message || error)}`);
  process.exitCode = 1;
}
